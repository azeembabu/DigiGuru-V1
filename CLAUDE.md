...
3. **NN-3 Daily 60-minute voice quota.** Sixty minutes of *active voice* per student per
   ...
   *Changed 2026-09-13 from a per-session hard cap to a daily resetting quota, and raised
   from twenty minutes to sixty on 2026-09-14, both on the product owner's instruction. The
   sixty-minute figure exists to fit a one-hour instructional module; a twenty-minute cap
   ended a lesson a third of the way through it. The enforcement point is unchanged: the
   ledger is server-side, the allowance counts BOTH voices (the tutor's speech bills the
   student too), and a session still ends with `end_reason = 'quota'` when the remaining
